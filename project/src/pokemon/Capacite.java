...
public class Capacite implements interfaces.ICapacite, interfaces.ICategorie, interfaces.IType {
    ...
    public int calculeDommage(interfaces.IPokemon lanceur, interfaces.IPokemon receveur) {
        ...
        switch (this.categorie.getNom()) {
            ...
            case "Vague Psy":
                if (!receveur.getEspece().getTypes()[0].getNom().equals("Ténèbres") || !receveur.getEspece().getTypes()[1].getNom().equals("Ténèbres")) {
        ...
        Pokedex pokedex = new Pokedex(); //Création d'une instance de la classe Pokedex pour acceder aux efficacités des pokémons
        ...
        return (int) (((((lanceur.getNiveau() * 0.4 + 2) * force * this.puissance) / (receveur.getStat().getDefense() * 50)) + 2) * CM); //Calcul du nombre de dommage
    }
